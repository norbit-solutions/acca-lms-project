...
export const adminApi = {
  ...
  // Chapters
  createChapter: (courseId: number, data: { title: string; sortOrder?: number }) =>
  ...
  // Lessons
  createLesson: (chapterId: number, data: {
    title: string
    sortOrder?: number
    isFree?: boolean
    maxViews?: number
  ...
  getLessonUploadUrl: (id: number) => api.post(`/admin/lessons/${id}/upload-url`),
  
  // Enrollments
  getEnrollments: (params?: { courseId?: number; userId?: number }) =>
    api.get('/admin/enrollments', { params }),
  createEnrollment: (data: { courseId: number; userId?: number; email?: string; phone?: string }) =>
  ...
  searchUsers: (query: string) => api.get('/admin/users/search', { params: { q: query } }),
