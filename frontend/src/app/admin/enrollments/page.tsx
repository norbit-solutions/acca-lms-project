...
export default function EnrollmentsPage() {
  ...
  const handleDelete = async (enrollment: Enrollment) => {
    if (!confirm(`Remove ${enrollment.user.fullName} from ${enrollment.course.title}?`)) return
    try {
              ...
              {enrollments.map((enrollment) => (
                <tr key={enrollment.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4">
                    ...
                    <p className="text-sm text-gray-400">{enrollment.user.phone}</p>
                  </td>
                  <td className="px-6 py-4 text-gray-900">{enrollment.course.title}</td>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl max-w-md w-full p-6">
            <h2 className="text-xl font-bold text-gray-900 mb-6">New Enrollment</h2>
